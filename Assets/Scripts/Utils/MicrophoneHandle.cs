...
public class MicrophoneHandle : MonoBehaviour {
	...
	public int requiedBlowTime = 0;    // How long a blow must last to be classified as a blow (and not a sigh for instance).
	public int limitTalkTime = 10;
	public int limitTapeTime = 6;
	...
	private void DeriveBlow() {
		...
		if (rmsValue > 0.01) {
			if (blowingTime > requiedBlowTime && pitchValue > 1300 && dbValue > -10) {
			...
			} else if (blowingTime <= limitTalkTime && pitchValue < 1400) {
				blowDisplay.GetComponent<Text> ().text = "Blowing";
